    ...
    public partial class ProjectOxford : IImageProcessing
    {
        public async Task<object> AnalyzeImage(string imagePath)
        {
            ...
            return await Task.Run(() =>
            {
                ...
                {
                    analysisResult = visionClient.AnalyzeImageAsync(stream, new string[] { "Color", "Faces" }).Result;
        ...
        {
            var analysis = (AnalysisResult)await AnalyzeImage(imagePath);
            return analysis.Faces[0].Age.ToString();
        }
    }
